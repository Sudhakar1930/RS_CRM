...
//				System.out.println("List Item Size:"+options.size());
	...
	public static boolean doClick(AndroidDriver driver, WebElement element) throws Exception {
		...
		try {
			...
			/*AndroidDriverWait webWait = new AndroidDriverWait(AndroidDriver, Duration.ofSeconds(20));
...
//			action.moveToElement(element).click().build().perform();
	...
	public static String getValue(AndroidDriver AndroidDriver, WebElement element) throws Exception{
		...
		try {
			/*AndroidDriverWait webWait = new AndroidDriverWait(AndroidDriver, Duration.ofSeconds(10));
			webWait.until(ExpectedConditions.elementToBeClickable(element));
			webWait.until(ExpectedConditions.visibilityOf(element));*/
			js.executeScript("arguments[0].scrollIntoView(true);", element);
...
//						tDateCols.get(j).click();
						utilities.UtilityCustomFunctions.doClick(driver, tDateCols.get(j));
